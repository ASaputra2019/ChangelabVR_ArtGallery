...
namespace ChangeLab.VRMusicAcademy
{
    ...
    {
        public static bool IsInRange(Transform source, Transform target, float range = 60f, float distance = 2)
        {
            Vector3 direction = (target.position - source.position).normalized;
            float dot = Vector3.Dot(source.forward, direction);
            float angle = Mathf.Acos(dot) * Mathf.Rad2Deg;
            return angle < range && Vector3.Distance(target.position,source.position) <= distance;
        }
